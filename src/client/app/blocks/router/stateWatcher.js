(function(){
    ...
    stateWatcher.$inject = ['$rootScope', 'myAuth', '$q', '$state', 'USER_ROLES', 'AUTH_EVENTS', '$localStorage'];
    function stateWatcher($rootScope, myAuth, $q, $state, USER_ROLES, AUTH_EVENTS, $localStorage){
        //default states
        var defaultStates = {
        ...
        function isStateAuthorized(event, toState, toParams, fromState, fromParams, options){
           ...
           if(toState.settings && toState.settings.roles){
            ...
            if(!myAuth.isAuthorized(toState.settings.roles)){
